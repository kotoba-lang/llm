(ns llm.app
  "etzhayyim-wasm-llm-llm8cf4ai appview — reagent + re-frame, view built from
  ...
  - `:app/relative-path` now names this file (relative to this repo's
    root, `orgs/kotoba-lang/llm/`), not the deleted Svelte one, and drops
    the stale `60-apps/etzhayyim-project-llm/` monorepo-era prefix the
    old constant carried — this repo's root already *is* that directory.
  - `:app/xrpc?` is now false, was true before this migration. This
    migration's wrangler.jsonc drops `main` entirely (see that file's
    header comment and the repo README): neither Worker source in this
    repo calls `env.ASSETS.fetch`, so the XRPC route this page used to
    advertise as enabled no longer deploys from this config. The XRPC
    handler itself is preserved byte-for-byte at
    `src/xrpc-proxy.ts` (moved, not deleted, from
  ...
  components or ext-rules change) with:
    ...
    (spit \"public/index.html\"
          (page/->page {:title \"etzhayyim-wasm-llm-llm8cf4ai\"
                         :lang \"ja\"
                         :description \"LLM Gateway — etzhayyim-wasm-llm-llm8cf4ai appview (reagent + re-frame + jp-go-dds).\"
                         :css (slurp (io/resource \"jp_go_dds/dds.css\"))}
                        [:div {:id \"app\"} \"etzhayyim-wasm-llm-llm8cf4ai loading…\"]
...
;; -- db ------------------------------------------------------------------
;;
;; Same nine facts `+page.svelte`'s `app` const held (title/project/name/
;; kind/routeCount/routes/vars/xrpc/relativePath).

(def default-db
  {:app/title "Llm Llm8cf4ai"
   :app/project "etzhayyim-project-llm"
   :app/name "etzhayyim-wasm-llm-llm8cf4ai"
   :app/kind "appview"
   :app/route-count 0
   :app/routes []
   :app/vars []
   :app/xrpc? false
   :app/relative-path "appview/etzhayyim-wasm-llm-llm8cf4ai/cljs/src/llm/app.cljs"})
